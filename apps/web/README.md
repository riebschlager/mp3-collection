# MP3 Collection Web

Astro frontend for browsing the MP3 archive and listening-history outputs.

## Routes

- `/`: overview dashboard with collection stats and most-played tracks
- `/artists`: artist index
- `/artists/[slug]`: artist detail page with grouped albums/tracks
- `/albums`: album index
- `/albums/[slug]`: album detail page with tracklist
- `/tracks/[page]`: paginated track browser
- `/history`: yearly/monthly listening timeline view
- `/history/anniversaries`: today/this-week-in-history explorer with manual month/day override
- `/history/artist-race`: weekly/monthly artist leaderboard animation with all-time and rolling-window modes
- `/history/month-wall`: poster-style month grid with filters and detail panel
- `/history/rediscovery`: arc visualization of dormant tracks returning after long gaps
- `/history/rediscovery-artists`: curated artist selector for rediscovery arcs
- `/history/rediscovery-artists/[slug]`: artist-specific rediscovery arcs
- `/wrapped`: wrapped-year gallery
- `/wrapped/[year]`: slide-based yearly wrapped story
- `/wrapped/monthly`: month gallery
- `/wrapped/monthly/[year]/[month]`: slide-based monthly wrapped story

## Data Source

The app reads static JSON from `public/data`, which is a symlink to the canonical repository data path:

```text
public/data -> ../../../data/derived/web
```

If the symlink is missing, recreate it from `apps/web/`:

```bash
ln -s ../../../data/derived/web public/data
```

Wrapped pages consume:
- `timeline.json` for yearly/monthly aggregate fallback data
- `anniversary-cache.json` for day- and week-based history lookups across past years
- `artist-race/index.json` plus `artist-race/*.json` variants for the weekly/monthly leaderboard animation
- `wrapped-stories.json` (generated by `go run . build-wrapped-stories` in `tools/pipeline`) for story-card and insight-driven content
- `wrapped-month-stories.json` (generated by `go run . build-wrapped-month-stories` in `tools/pipeline`) for month-level story pages

## Commands

Run from `apps/web/`:

```bash
npm install
npm run dev
```

Additional commands:
- `npm run build`: production build to `dist/`
- `npm run preview`: preview production build
- `npm run astro -- --help`: Astro CLI help

## GitHub Pages Configuration

`astro.config.mjs` is configured for GitHub Pages:
- `site: 'https://riebschlager.github.io'`
- `base: '/mp3-collection'`
- `output: 'static'`

Route links in the app use `import.meta.env.BASE_URL` helpers to stay base-path safe.

## Current Implementation Notes

- Several dynamic pages currently assume `49` track chunks:
  - `src/pages/tracks/[page].astro`
  - `src/pages/artists/[slug].astro`
  - `src/pages/albums/[slug].astro`
- If `data/derived/web/chunks` count changes, update those constants to match.
